...
export default function AppSelectorModal({ isOpen, onClose, onSelect }: AppSelectorModalProps) {
    ...
    const mainApp = APPS[0] // Happ
    const alternativeApps = APPS.slice(1) // Остальные приложения

    const filteredAlternatives = alternativeApps.filter(app => {
                ...
                <div className="flex-1 overflow-y-auto p-4">
                    <div className="grid gap-3">
                        ...
                        <button
                            ...
                            className="p-3 bg-slate-700/40 hover:bg-slate-700/60 border border-slate-600/30 rounded-xl transition-all duration-200 text-left flex items-center justify-between"
                        >
                        ...
                        {showAlternatives && filteredAlternatives.map((app) => (
                            <button
                                key={app.id}
                                onClick={() => {
                                    onSelect(app.id)
                                ...
                                className="p-4 bg-slate-800/40 hover:bg-slate-700/60 border border-green-700/10 hover:border-green-600/30 rounded-xl transition-all duration-200 hover:scale-[1.02] text-left group"
                            >
                                <div className="flex items-start gap-4">
